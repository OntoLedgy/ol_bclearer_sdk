...
def __get_universe_nf_uuids_to_ea_guids_table(
    ...
    for (
        collection_type,
        collection,
    ) in content_universe.nf_ea_com_registry.dictionary_of_collections.items():
...
def __add_nf_uuids_to_ea_guids_if_required(
    ...
    new_mappings = new_mappings.loc[
        new_mappings[NfEaComColumnTypes.EXPLICIT_OBJECTS_EA_GUID.column_name]
        != DEFAULT_NULL_VALUE
    ]
...
def __get_secondary_to_primary_universe_nf_uuids_map(
    ...
    key_column = NfColumnTypes.NF_UUIDS.column_name + DEFAULT_MASTER_TABLE_SUFFIX

    value_column = NfColumnTypes.NF_UUIDS.column_name + DEFAULT_FOREIGN_TABLE_SUFFIX
    ...
    secondary_to_primary_universe_nf_uuids_map = (
        matched_by_ea_guid_nf_uuids_dataframe_copy.set_index(
            key_column,
        )[value_column].to_dict()
    )
