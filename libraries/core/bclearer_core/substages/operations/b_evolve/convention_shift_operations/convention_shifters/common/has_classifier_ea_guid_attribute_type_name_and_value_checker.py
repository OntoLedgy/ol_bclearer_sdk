...
def has_classifier_ea_guid_attribute_type_name_and_value(
    ...
    classifier_ea_guid_has_attribute_type_name_and_value = False
...
def __check_attribute_and_type_is_a_match(
    ...
    current_attribute_type_name_result = content_universe_ea_classifiers[
        content_universe_ea_classifiers[nf_uuids_column_name]
        == current_attribute_type_nf_uuid_string
    ][ea_object_name_column_name]

    if not current_attribute_type_name_result.empty:
        current_attribute_type_name = current_attribute_type_name_result.values[0]

        if (
            attribute_value == current_attribute_value
            and attribute_type_name == current_attribute_type_name
        ):
            return True
