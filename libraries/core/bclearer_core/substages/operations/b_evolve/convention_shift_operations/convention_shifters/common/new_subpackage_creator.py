...
from nf_ea_common_tools_source.b_code.nf_ea_common.common_knowledge.column_types.ea_t.ea_t_package_column_types import (
...
def create_new_subpackage_if_not_exist(
    ...
    list_of_first_level_contained_packages = nf_ea_com_universe_ea_packages[
        nf_ea_com_universe_ea_packages[nf_uuids_column_name] == package_nf_uuid
    ][contained_ea_packages_column_name].tolist()[0]

    ea_packages_sliced_to_first_level_contained_packages = (
        nf_ea_com_universe_ea_packages[
            nf_ea_com_universe_ea_packages[nf_uuids_column_name].isin(
    ...
    name_package_already_created_in_parent_package_nf_uuid = None
    ...
    for new_ea_packages_row_dictionary in new_ea_packages_dictionary.values():
        if (
            ...
            == package_nf_uuid
            and new_ea_packages_row_dictionary[object_name_column_name]
            == new_subpackage_name
        ):
            name_package_already_created_in_parent_package_nf_uuid = (
                new_ea_packages_row_dictionary[nf_uuids_column_name]
            )
    ...
    if new_subpackage_name in list_of_first_level_contained_packages_names:
        ...
        new_subpackage_nf_uuid = (
            new_subpackage_nf_uuid_row[nf_uuids_column_name]
    ...
    if name_package_already_created_in_parent_package_nf_uuid is not None:
...
def __add_new_package_row(
    ...
    new_package_nf_uuid = create_new_uuid()

    new_package_ea_guid = create_ea_guid_from_nf_uuid(
        nf_uuid=new_package_nf_uuid,
    )

    parent_package_ea_guid = (
        ea_packages_table[
            ea_packages_table[nf_uuids_column_name] == parent_package_nf_uuid
        ][ea_guid_column_name]
        .to_string(index=False)
        .strip()
    )
