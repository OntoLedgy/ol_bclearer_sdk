...
from bclearer_interop_services.excel_services.interop.excel_write import save_table_in_excel
...
class NfRegistries:
    ...
    def export_dataframes_to_new_database(
        ...
        output_csv_folder = \
            output_database_folder.joinpath(
                'csvs')
        ...
        if add_xlsx_export:
            ...
            self.export_dataframes(
                short_name=short_name,
                output_folder_name=str(output_csv_folder) + os.sep)
        ...
        filenames = \
            [f for f in os.listdir(output_csv_folder) if isfile(join(output_csv_folder, f))]
    ...
    def __export_dataframes_as_csv_and_xlsx(
        ...
        for collection_type, dataframe in self.dictionary_of_collections.items():
    ...
    def __export_dataframe_as_csv_and_xlsx(
        ...
        if not isinstance(collection_type, CollectionTypes):
            raise \
                TypeError
        ...
        if len(collection_name) + len(short_name) > 59:
            collection_name = \
                collection_name[0:59 - len(short_name)]
    ...
    def export_dataframes(
        ...
        for collection_type, dataframe in self.dictionary_of_collections.items():
            ...
            dataframe.to_csv(
                output_folder_name + short_name + '_' + str(collection_type.collection_name) + '.csv',
                sep=',',
                quotechar='"',
                index=False,
                quoting=csv.QUOTE_ALL)
    ...
    def create_or_update_summary_table(
        ...
        for collection_type, table in self.dictionary_of_collections.items():
            if not isinstance(collection_type, CollectionTypes):
                raise \
                    TypeError

            summary_dictionary = \
                self.__add_counts_to_dictionary(
    ...
    def update(
        ...
        merged_collections = \
            concat(
                objs=[current_collection, new_collection],
                ignore_index=True,
