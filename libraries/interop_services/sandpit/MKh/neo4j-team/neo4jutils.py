...
class Neo4jLoader(threading.Thread):
    ...
    def run(self):
        ...
        try:
                ...
                if len(self.queue) > 0:
                    ...
                    if (
                        ...
                        if (
                            ...
                            self.relationshipsCreated += (
                                cntr.relationships_created
                            )
                            self.pcnt = (
                                self.pcnt
                                + len(
            ...
            print(
                f"loader {self.threadID} Cancelled after crtl-c",
            )
            sys.exit()
        except Exception as e:
            print(
                f" last batch unexpected error one line {self.line_count}",
            )
            print(type(e))
            print(e.args)
            print(len(self.rows))
...
def run_cypher(
    ...
    if cntr.indexes_removed > 0:
        df2.loc[len(df2.index)] = [
            "indexes removed",
            cntr.indexes_removed,
        ]
    if cntr.constraints_added > 0:
        df2.loc[len(df2.index)] = [
            "constraints added",
            cntr.constraints_added,
        ]
    if cntr.constraints_removed > 0:
        df2.loc[len(df2.index)] = [
            "constraints removed",
            cntr.constraints_removed,
        ]
...
def loadmysqltoneo4j(
    ...
    print(
        f"                         batch size: {aBatchSize}",
    )
    print(
        f"loadmysqltoneo4j started connection:{SQLconnection}",
    )
    if debug:
        print(
            f"                           SQLQuery: {SQLQuery}",
    ...
    print(
        f"                     neo4j database: {aDBName}",
    )
    print(
        f"                      neo4j address: {aDriver.get_server_info().address}",
    )
    ...
    if debug:
        print(
            f" labelsCreated: {labelsCreated}, nodesCreated: {nodesCreated}, propertiesSet: {propertiesSet}, relationshipsCreated: {relationshipsCreated}",
        )
...
def loadcsvtoneo4j(
    ...
    print(
        f"                 batchSize: {aBatchSize} ",
    )
    print(
        f"                 delimiter: {aDelimiter} ",
    )
    print(
        f"                 quotechar: {aQuotechar} ",
    )
    print(
        f"                escapechar: {escapeChar} ",
    )
