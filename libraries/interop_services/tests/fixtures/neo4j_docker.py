...
def neo4j_container():
    ...
    time.sleep(15)  # Increase the wait time if needed
    logs = neo4j_container.logs()
    print(logs.decode("utf-8"))  # Print logs to ensure the neo4j_container is running
...
@pytest.fixture(scope="session")
def neo4j_docker_driver(neo4j_container):
    from neo4j import GraphDatabase

    uri = "bolt://localhost:7687"

    neo4j_docker_driver = GraphDatabase.driver(uri, auth=("neo4j", "testpassword"))
...
def start_neo4j_container():
    ...
    time.sleep(15)
    logs = container.logs()
    print("Neo4j container logs:", logs.decode("utf-8"))

    # Yield the container for interaction
    yield container

    # Do not stop or remove the container to leave it running
    print("Neo4j container is still running for interaction.")

@pytest.fixture(scope="session")
def neo4j_shutdown_container():
    client = docker.from_env()
    container = client.containers.get("open_neo4j")
    ...
    container.stop()
    container.remove()
    print("Neo4j container has been shut down and removed.")

@pytest.fixture(scope="session")
def neo4j_docker_connection(neo4j_docker_driver):
