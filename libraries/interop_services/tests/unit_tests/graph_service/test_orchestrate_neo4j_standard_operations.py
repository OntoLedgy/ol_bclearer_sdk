...
class TestNeo4jStandardOperations():
    ...
    def test_standard_query_execution(
        ...
        records = neo4j_connection.execute_cypher_query(self.cypher_query)
    ...
    def test_query_list_execution(
        ...
        self.cypher_query_list = generate_list_from_text(self.cypher_queries)

        for item in self.cypher_query_list:
            try:            
                with neo4j_connection.get_new_session(neo4j_connection.database_name) as session:            
                    if len(item)>0:
                                            
                        records = session.execute_cypher_query(item)
    
                    print(records)                             
            except:
                pass

    def test_load_dataset_generation(self):
    ...
    def test_load_dataset_by_category_generation(self):
