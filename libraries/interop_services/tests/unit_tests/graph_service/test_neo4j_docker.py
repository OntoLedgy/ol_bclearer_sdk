...
class TestNeo4jDocker:
    ...
    def test_neo4j_deploy_and_keep_open(self,start_neo4j_container):
        # You can now interact with the running Neo4j instance
        print("Neo4j container is up and running for further interaction.")

        # Test to shutdown Neo4j container on demand
    def test_neo4j_shutdown(self, neo4j_shutdown_container):
        print("Shutting down Neo4j container.")
    ...
    def test_with_custom_connection(
        ...
        result = session.execute_cypher_query("RETURN 1 AS number")
        record = result[0]
        assert record["number"] == 1
