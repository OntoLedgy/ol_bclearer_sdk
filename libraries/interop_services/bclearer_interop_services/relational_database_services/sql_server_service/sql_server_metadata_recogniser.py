...
def find_all_tables_from_schema_via_regex(
    ...
    for table_pattern_match in re.finditer(
        table_compiled_pattern,
        schema,
    ):
        table_name = table_pattern_match.group(1)

        col_data = table_pattern_match.group(2)

        for col_pattern_match in re.finditer(
            col_compiled_pattern,
            col_data,
        ):
            col_name = col_pattern_match.group(
                1,
            )

            col_type = col_pattern_match.group(
                2,
            )
