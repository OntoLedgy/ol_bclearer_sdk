...
class Workbooks:
    def __init__(
        ...
        if file_path:
            ...
            self.sheets = {sheet.title: Sheets(sheet) for sheet in self.wb.worksheets}
    ...
    def _load_xls(self, file_path):
        ...
        for sheet_name in xls.sheet_names:
            ...
            self.sheets[sheet_name] = self._convert_df_to_sheet(df, sheet_name)

    def _convert_df_to_sheet(self, dataframe, sheet_name):
        """
        Converts a pandas DataFrame to the internal Sheets object for .xls files.
        """
        openpyxl_sheet = OpenpyxlWorkbook().create_sheet(sheet_name)

        openpyxl_sheet.append(dataframe.columns.tolist())

        for r_idx, row in dataframe.iterrows():
            openpyxl_sheet.append(row.tolist())
    ...
    def _save_xls(self, file_path):
        with pd.ExcelWriter(file_path, engine='xlwt') as writer:
            for sheet_name, sheet in self.sheets.items():
                dataframe = pd.DataFrame([[cell.value for cell in row] for row in sheet.sheet.rows])
    ...
    def create_sheet(self, title: str):
        ...
        openpyxl_sheet = self.wb.create_sheet(title=title)
