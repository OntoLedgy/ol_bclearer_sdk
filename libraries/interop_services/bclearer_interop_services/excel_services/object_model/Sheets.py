...
class Sheets:
    def __init__(self, sheet: OpenpyxlWorksheet):
        self.sheet = sheet

    def cell(self, row: int, col: int):
        return Cells(self.sheet.cell(row=row, column=col))

    def row(self, index: int):
        return Rows(self.sheet, index)

    def column(self, index: int):
        return Columns(self.sheet, index)

    def range(self, min_row: int, min_col: int, max_row: int, max_col: int):
        return Ranges(self.sheet, min_row, min_col, max_row, max_col)

    def read_to_dataframe(self, header_row_number: int = 1) -> pd.DataFrame:
        ...
        while header_index < len(data) and not any(data[header_index]):
            header_index += 1  # Skip empty rows

        # Ensure we are within bounds
        if header_index >= len(data):
            raise ValueError(f"header_row_number {header_row_number} (adjusted to {header_index + 1}) is out of range.")
