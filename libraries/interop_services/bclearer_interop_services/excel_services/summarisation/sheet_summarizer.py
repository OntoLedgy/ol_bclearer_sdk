...
def sheet_summariser(
    ...
    def csv_sheet_summariser(
            file_name):

        csv_data = pd.read_csv(os.path.join(path, file_name), encoding='latin-1')

        csv_data = csv_data.dropna(how='all').reset_index(drop=True)
    ...
    def excel_sheet_summariser(
        ...
        if sheet_rows > 0:
            ...
            sheet_data_dataframe = \
                sheet_data_dataframe.dropna(how='all').reset_index(drop=True)
            ...
            print(f'found columns: {column_list}\n')
        else:
            excel_sheet_summary_dataframe = pd.DataFrame({
                "number_of_columns": [0],
                "number_of_rows": [0]
    ...
    if file_extension == ".csv":
        ...
        sheet_summary_df["sheet_names"] = os.path.basename(file_path_and_name)

    else:
        if (file_extension == ".xlsx" or file_extension == ".xlsm"):
            workbook = load_workbook(os.path.join(path, file_path_and_name))

            sheet_names_list = workbook.sheetnames

            number_of_sheets = len(sheet_names_list)

        else:
            workbook = open_workbook(os.path.join(path, file_path_and_name))
...
def get_sheet_summary_from_directory(
    ...
    for file_name in files:
        ...
        if file_extension in valid_file_extensions:
            print(f'*********summarising file {file_name} in {parent_directory_path}**********\n')
...
def summarise_sheet_files_in_directory(
        ...
        valid_file_extensions) -> pd.DataFrame:
    ...
    for parent_directory_path, sub_directories, files in os.walk(directory_path_and_name):
