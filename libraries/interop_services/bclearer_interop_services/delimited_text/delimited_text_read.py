...
from bclearer_interop_services.file_system_service.encoding.file_encoding_detector import detect


def get_table_from_csv_with_header(
        relative_filename: Union[str, os.PathLike],
        file_encoding: str,
        sep: str = ",",
...
def __check_if_read_was_successful(
    ...
    read_file_relative_filename = \
        source_relative_filename.replace(
            '.csv',
            '_read.csv')
    ...
    sys.exit(
        'Load was terminated because data was corrupted while reading from ' + source_relative_filename)
