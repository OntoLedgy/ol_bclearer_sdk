...
class Folders(
    ...
    def __init__(
        ...
        if parent_folder and not isinstance(parent_folder, Folders):
            raise \
                TypeError
    ...
    def get_file_count(
            ...
            -> int:
        return \
            len(self.child_files)

    def get_folder_count(
            self) \
            -> int:
        return \
            len(self.child_folders)

    def populate_folder_length_in_bytes(
            self) \
            -> None:
        descendants_list = \
            list(Path(self.absolute_path_string).rglob('*'))
    ...
    def get_folder_information_as_b_dataset_row_dictionary(
        ...
        if not self.parent_folder:
            ...
            relative_path = \
                self.absolute_path_string.replace(
        ...
        folder_as_b_dataset_row_dictionary = \
            {
                'uuid': self.uuid,
                'file_system_object_type': type(self).__name__,
                'file_count': str(self.get_file_count()),
                'folder_count': str(self.get_folder_count()),
                'full_name': self.absolute_path_string,
                'base_name': self.base_name,
                'length': str(self.file_system_object_properties.length),
