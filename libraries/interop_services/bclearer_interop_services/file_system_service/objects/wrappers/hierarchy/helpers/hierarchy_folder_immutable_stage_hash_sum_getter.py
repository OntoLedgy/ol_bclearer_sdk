...
def get_hierarchy_folder_immutable_stage_hash_sum(
    ...
    if not isinstance(hierarchy_file_system_object_universe_register, HierarchyFileSystemObjectRegisters):
        raise \
            TypeError
    ...
    for child_hierarchy_file_system_object \
            in child_hierarchy_file_system_objects:
        child_immutable_stage_content_hash = \
            __get_child_immutable_stage_content_hash(
...
def __get_child_immutable_stage_content_hash(
    ...
    child_file_system_object = \
        hierarchy_file_system_object_universe_register.get_file_system_object_from_hierarchy_object(
            hierarchy_file_system_object=child_hierarchy_file_system_object)

    if isinstance(child_file_system_object, Files):
        return \
            child_file_system_object.file_immutable_stage_hash

    elif isinstance(child_hierarchy_file_system_object, HierarchyFolders):
        hierarchy_folder_immutable_stage_content_hash_sum = \
