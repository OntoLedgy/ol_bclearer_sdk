...
class HierarchyFileSystemObjectRegisters:
    ...
    def add_hierarchy_folder_to_hierarchy(
            ...
            -> None:
        folder = \
            self.get_file_system_object_from_hierarchy_object(
                hierarchy_file_system_object=hierarchy_folder)

        first_level_children_paths = \
            get_first_level_children_file_system_object_paths(
    ...
    def add_hierarchy_object_to_file_system_object_map_to_register(
            ...
            -> None:
        self.hierarchy_objects_to_file_system_objects_mapping[hierarchy_file_system_object] = \
            file_system_object
    ...
    def get_hierarchy_object_from_file_system_object(
            ...
            -> [HierarchyFolders, HierarchyFileSystemObjects]:
    ...
    def __populate_folder_lengths_in_bytes(
        ...
        for hierarchy_file_system_object, file_system_object \
                in self.hierarchy_objects_to_file_system_objects_mapping.items():
            if isinstance(file_system_object, Folders):
    ...
    def export_register_in_b_datasets_format(
        ...
        if isinstance(self.root, HierarchyFolders):
            add_hierarchy_folder_to_b_dataset_format(
                hierarchy_file_system_object_register=self,
                hierarchy_folder=self.root,
    ...
    def get_relative_path(
        ...
        root_path = \
            self.get_file_system_object_from_hierarchy_object(
                hierarchy_file_system_object=self.root).absolute_path_string

        relative_path = \
            os.path.relpath(
                path=self.get_file_system_object_from_hierarchy_object(
                    hierarchy_file_system_object=hierarchy_file_system_object).absolute_path_string,
                start=root_path)
