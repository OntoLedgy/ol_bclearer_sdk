...
def get_file_system_objects_from_paths(
    ...
    for path in \
            paths:
        file_system_object = \
            get_file_system_object_from_path(
                path=path,
                base_root_path=base_root_path)

        if not isinstance(file_system_object, Folders):
            raise \
                TypeError

        if not os.path.exists(file_system_object.absolute_path_string):
            raise \
                FileNotFoundError
...
def get_file_system_object_from_path(
    ...
    elif os.path.exists(absolute_path) and os.path.isfile(absolute_path):
        file_system_object = \
            Files(
                absolute_path_string=absolute_path)
