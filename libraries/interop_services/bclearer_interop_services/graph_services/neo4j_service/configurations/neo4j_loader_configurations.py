...
class CSVConfigurations:
    def __init__(self, config: dict):
        self.csv_path: str = ""
        self.csv_file_delimiter: str = config.get(
            "csv_file_delimiter",
            ",",
        )
        self.csv_file_quote_character: str = config.get(
            "csv_file_quote_character",
            '"',
        )
        self.csv_file_encoding: str = config.get(
            "csv_file_encoding",
            "utf-8",
        )
        self.csv_escape_character: str = config.get(
...
class Neo4jLoaderConfigurations:
    def __init__(
        ...
        with open(
            ...
            self.batch_size: int = json_model.get(
                "batch_size",
                1000,
            )
            self.concurrency: int = json_model.get(
                "concurrency",
                4,
            )
            ...
            self.csv_configurations: CSVConfigurations = CSVConfigurations(
                json_model.get(
                    "csv_configurations",
                    {},
                ),
            )
            self.emptyIsNull: bool = json_model.get(
                "emptyIsNull",
                False,
            )
            self.debug: bool = json_model.get(
                "debug",
                False,
            )
    ...
    def print_configuration(self):
        print(
            "loadCSVToNeo4j started file: " + self.csv_configurations.csv_path,
        )
        print(
