    ...
    def recurse(structure, path=''):
        for key, value in structure.items():
            if isinstance(value, dict):
                ...
                if 'data' in value and 'cypher' in value:
                    pair = {'data': value['data'], 'cypher': value['cypher']}
...
def get_load_dataset(parent_folder):
    structure =generate_load_dataset_from_folder(parent_folder)  
    load_dataset = iterate_structure(structure)      
    return load_dataset    

def get_load_dataset_by_graph_object_type(parent_folder):
    
    structure = generate_load_dataset_from_folder(parent_folder)
    
    nodes, edges = categorize_structure(structure)
