from networkx import is_isomorphic, DiGraph, difference

from bclearer_interop_services.graph_services.network_service.graph_transitive_reductor \
    import transitively_reduce_graph
...
def is_graph_minimal(
        ...
        graph_name: str) \
        -> bool:
    transitively_reduced_graph = \
        transitively_reduce_graph(
            directed_graph=directed_graph,
            graph_name=graph_name)

    graph_and_reduction_are_isomorphic = \
        is_isomorphic(
            G1=directed_graph,
    ...
    extra_edges = \
        list(difference(
            G=directed_graph,
            H=transitively_reduced_graph).edges())

    if not graph_and_reduction_are_isomorphic:
        __try_logging_message(
            message='WARNING: GRAPH \'' + graph_name + '\' IS NOT THE MINIMAL POSSIBLE GRAPH\n'
                    + 'Extra edges: ' + str(extra_edges))

    else:
        __try_logging_message(
            message='INFORMATION: Graph \'' + graph_name + '\' is the (transitive) minimal possible graph_service')
