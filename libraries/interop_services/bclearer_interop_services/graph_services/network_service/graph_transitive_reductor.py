...
from bclearer_interop_services.graph_services.network_service.directed_graph_cycles_checker import check_directed_graph_cycles
...
def transitively_reduce_graph(
    ...
    if directed_graph_cycles_list:
        sys.exit(
            'ERROR: CYCLES FOUND IN DIRECTED GRAPH: ' + graph_name
            + '\n' + str(directed_graph_cycles_list))

    transitively_reduced_directed_graph = \
    ...
    transitively_reduced_directed_graph.add_edges_from(
        (source, target, directed_graph.edges[source, target])
