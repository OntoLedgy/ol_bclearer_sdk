...
def report_tree_node_level(
    ...
    tree_node_superior = pd.Series(tree_dataframe[superior_node_column_name].values, index=tree_dataframe[tree_node_column_name]).to_dict()

    tree_dataframe['list_of_superior_nodes'] = tree_dataframe[tree_node_column_name].apply(
        lambda x: list_tree_node_superiors(tree_node=x, superior_node_list=tree_node_superior)
    )
    tree_dataframe['num_list_superior_nodes'] = tree_dataframe["list_superior_nodes"].apply(len)
