...
def report_ancestors(
    ...
    if tree_node in parent_tree_nodes.keys():
        ...
        return [parent] + report_ancestors(parent, parent_tree_nodes=parent_tree_nodes)
    else:
        return []
...
def check_tree_level(
    ...
    d_parent_nodes = \
        pd.Series(
            df[parent_column_name].
            values, 
            index=df[node_column_name]).to_dict()
    
    df['list_ancestors'] = df[node_column_name].apply(
        lambda x: report_ancestors(tree_node=x, parent_tree_nodes=d_parent_nodes)
    )
