...
from bclearer_orchestration_services.identification_services.hash_service.hash_creator import create_identity_hash_string
...
def export_pyspark_dataframe_as_parquet_table_using_pyarrow(
    ...
    pandas_dataframe.to_parquet(
        path=output_root_folder.absolute_path_string + os.sep + str(uuid) + '.snappy.parquet_service',
        engine='pyarrow',
        compression='snappy')
