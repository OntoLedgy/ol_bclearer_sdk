...
class BEngWorkspaceFileSystemObjects(
    ...
    def get_code_folders(
        ...
        code_folder_components = \
            self.list_of_components()[code_start_index:]

        if code_folder_components[-1].endswith('.py'):
            code_folder_components = \
                code_folder_components[:-1]

        relative_code_folder_path_strings = \
            []

        relative_path_from_workspace_string = \
            '/'.join(self.list_of_components()[b_eng_workspace_folder_index:code_start_index])

        for code_folder_component in code_folder_components:
            relative_path_from_workspace_string = \
                '/'.join([relative_path_from_workspace_string, code_folder_component])
    ...
    def get_reference_from_b_eng_workspace_file_system_object(
        ...
        reference_string = \
            '.'.join(code_folder_components)

        if reference_string.endswith('.py'):
            reference_string = \
                reference_string[:-3]
