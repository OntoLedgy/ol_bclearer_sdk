...
from nf_common.code.services.b_eng_python_refactoring_service.objects.b_eng_file_system_objects.b_eng_project_folders import \
    BEngProjectFolders
from nf_common.code.services.version_control_services.git_service.commit_and_pusher import add_commit_and_push_local_repository
from nf_common.code.services.b_eng_python_refactoring_service.objects.b_eng_file_system_objects.b_eng_project_folder_lists import BEngProjectFolderLists
...
def __commit_refactors_in_b_eng_project_folder(
    ...
    folder_paths = \
        [os_object.path for os_object in os.scandir(str(local_git_repositories_folder_name)) if os_object.is_dir()]
...
def __commit_refactors_in_git_repository(
    ...
    add_commit_and_push_local_repository(
        repository_folder_path=Path(local_git_repository_folder_path),
        branch_name=branch_name,
        commit_message=commit_message)
