from storage_interop_services_source.code.constants import LOCAL_GIT_REPOSITORY_FOLDER_NAME
from storage_interop_services_source.code.constants import CODE_FOLDER_NAME
from storage_interop_services_source.code.constants import SANDPIT_FOLDER_NAME
from storage_interop_services_source.code.constants import EXECUTABLES_FOLDER_NAME
from storage_interop_services_source.code.constants import FOLDER_DELIMITER
...
def get_b_eng_folder_type(
        ...
        parent_folder: BEngFolders) -> EnumBEngFolderTypes:
    if __is_bitbucket_team_folder(parent_folder=parent_folder):
        return \
    ...
    if __is_git_repositories_folder(child_folder_name=child_folder_name):
        return \
            EnumBEngFolderTypes.GIT_REPOSITORIES

    if parent_folder.b_eng_folder_type is EnumBEngFolderTypes.GIT_REPOSITORIES:
        return __get_git_repository_folder_type(child_folder_name=child_folder_name)
    ...
    if child_folder_name.endswith(FOLDER_DELIMITER + CODE_FOLDER_NAME + FOLDER_DELIMITER):
        return \
            EnumBEngFolderTypes.CODE_FOLDER

    if child_folder_name.endswith(FOLDER_DELIMITER + SANDPIT_FOLDER_NAME + FOLDER_DELIMITER):
        return \
            EnumBEngFolderTypes.SANDPIT_FOLDER

    if child_folder_name.endswith(FOLDER_DELIMITER + EXECUTABLES_FOLDER_NAME + FOLDER_DELIMITER):
        return \
    ...
    if parent_folder.b_eng_folder_type is EnumBEngFolderTypes.DATABASE_OUTPUTS_REPOSITORY:
        return \
    ...
    return \
        EnumBEngFolderTypes.NOT_SET


def __is_bitbucket_team_folder(
        parent_folder: BEngFolders)\
        -> bool:
    if isinstance(parent_folder, BEngWorkspaceFolders):
        return \
            True
...
def __get_git_repository_folder_type(
        child_folder_name: str)\
        -> EnumBEngFolderTypes:
    if child_folder_name.endswith('_cd' + FOLDER_DELIMITER):
        return \
            EnumBEngFolderTypes.CREATED_DATA_REPOSITORY

    if child_folder_name.endswith('_od' + FOLDER_DELIMITER):
        return \
            EnumBEngFolderTypes.ORIGINAL_DATA_REPOSITORY

    if child_folder_name.endswith('_database_templates' + FOLDER_DELIMITER):
        return \
