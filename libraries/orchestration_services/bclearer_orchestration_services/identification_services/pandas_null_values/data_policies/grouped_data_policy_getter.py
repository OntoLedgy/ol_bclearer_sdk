...
def get_grouped_data_policy(
    ...
    for column_name \
            in data_policy_counts_as_table.columns:
...
def __add_grouped_dictionary(
    ...
    unique_data_policy_names = \
        data_policy_counts_as_table[column_name].unique()

    for unique_data_policy_name \
            in unique_data_policy_names:
        count = \
            len(list((
                data_policy_counts_as_table[data_policy_counts_as_table[column_name] == unique_data_policy_name][
                    column_name])))
