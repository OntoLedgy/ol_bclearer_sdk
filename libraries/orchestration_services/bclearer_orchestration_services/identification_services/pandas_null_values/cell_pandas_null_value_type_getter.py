...
def get_cell_pandas_null_value_type(
    ...
    try:
        ...
        log_message(message='ERROR occurred trying to process a data policy for cell value: {0} - {1}'.format(
            str(cell),
            str(error)))
        return \
            PandasNullValueTypes.NOT_SET
...
def __get_null_data_policy(
        cell) \
        -> PandasNullValueTypes:
    for null_data_policy \
            in PandasNullValueTypes:
        if null_data_policy is PandasNullValueTypes.NOT_SET \
                or not isinstance(null_data_policy, PandasNullValueTypes):
            continue
        null_data_policy_value_type = \
            type(
                null_data_policy.value)
        # TODO: Need to check we can capture all the null data policies
        if isinstance(cell, null_data_policy_value_type):
            match null_data_policy_value_type.__name__:
                ...
                case 'float':
                    if numpy.isnan(cell) \
                            and numpy.isnan(null_data_policy.value):
                        return \
                            null_data_policy
