from nf_common.code.services.identification_services.b_identity_service.b_identity_creators.b_identity_base_from_string_creator import \
    create_b_identity_base_from_string
from nf_common.code.services.identification_services.b_identity_service.b_identity_creators.b_identity_from_sorted_list_of_b_identities_creator import \
    create_b_identity_from_sorted_list_of_b_identities
from nf_common.code.services.identification_services.b_identity_service.b_identity_creators.b_identity_sum_from_b_identities_creator import \
...
def create_b_identity_using_role_based_ids(
        ...
        -> int:
    b_identity_role_components = \
        list()

    for b_identity_component \
...
def __add_component_to_components(
    ...
    role_type = \
        b_identity_component[0]

    role_value = \
        b_identity_component[1]

    if not isinstance(role_type, role_based_type_instance):
        raise \
            TypeError
    ...
    role_based_b_identity = \
        create_b_identity_sum_from_b_identities(
            b_identities=[role_type_b_identity, role_value])
