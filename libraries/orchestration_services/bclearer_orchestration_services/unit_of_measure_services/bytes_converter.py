...
def convert_bytes_to_byte_units(
    ...
    bytes_exponent_value = \
        math.log(bytes, 2)

    bytes_unit_value = \
        __get_byte_unit_value(
            bytes=bytes,
            bytes_exponent_value=bytes_exponent_value)

    byte_unit_suffix = \
        __get_byte_unit_suffix(
            bytes_exponent_value=bytes_exponent_value)

    byte_units = \
        bytes_unit_value + ' ' + byte_unit_suffix
...
def __get_byte_unit_suffix(
        bytes_exponent_value: float) -> str:
    bytes_magnitude_order = \
        int(bytes_exponent_value // 10)
...
def __get_byte_unit_value(
    ...
    bytes_multiple_value = \
        bytes / (2 ** (10 * bytes_magnitude_order))
    ...
    byte_unit_value = \
        str(rounded_bytes_multiple_value)
