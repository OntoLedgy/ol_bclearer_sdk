...
def log_timing_header():
    ...
    log_message(
        message=NAME_VALUE_DELIMITER.join(column_names))
...
def __get_standard_list(
    ...
    total_memory = \
        f'{__get_gigabytes(byte_count=svmem.total):.2f}'

    available_memory = \
        f'{__get_gigabytes(byte_count=svmem.available):.2f}'

    used_memory = \
        f'{__get_gigabytes(byte_count=svmem.used):.2f}'
