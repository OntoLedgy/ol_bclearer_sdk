...
def convert_raw_radon_json_to_dataframe(
    ...
    for full_file_path, result in results_as_json.items():
        raw_dataframe = \
            __append_result_to_dataframe(
                full_file_path=full_file_path,
                result=result,
                raw_dataframe=raw_dataframe)

    ordered_raw_dataframe = \
        raw_dataframe.sort_values(
            by=['total_lines_of_code_count'],
            ascending=False)
...
def __append_result_to_dataframe(
    ...
    raw_dataframe = \
        raw_dataframe.append(
            {
                'file_uuid': file_uuid,
                'full_file_path': full_file_path,
                'total_lines_of_code_count': result['loc'],
                'logical_lines_of_code_count': result['lloc'],
                'source_lines_of_code_count': result['sloc'],
                'lines_of_comments_count': result['comments'],
                'lines_of_multi_line_strings_count': result['multi'],
                'blank_lines_count': result['blank'],
                'single_lines_of_comments_count': result['single_comments']
            },
            ignore_index=True)
