...
def analyse_code_using_pylint(
        b_pylint_configuration: PylintConfigurations) -> dict:
    pylint_options_string = \
        __get_pylint_options_string_from_configuration(
            b_pylint_configuration=b_pylint_configuration)

    pylint_results_as_json = \
        __get_pylint_analysis_as_json(
...
def __get_pylint_options_string_from_configuration(
        b_pylint_configuration: PylintConfigurations) -> str:

    input_file_system_object_paths_list = \
        []

    for input_file_system_object in b_pylint_configuration.common_configuration.input_file_system_objects:
    ...
    pylint_options = \
        ...
        [
            ...
            '--disable=' + ','.join(b_pylint_configuration.excluded_pylint_check_codes)
        ]
...
def __get_pylint_analysis_as_json(
        pylint_options_string: str) -> json:
    log_message(
        message='Code is being analysed by pylint.')

    pylint_analysis_output, pylint_analysis_errors = \
        epylint.py_run(
            pylint_options_string,
            return_std=True)
...
def __transform_pylint_results_json_to_dictionary_of_pylint_result_dataframes(
        pylint_results_as_json: json) -> dict:
    pylint_results_as_dataframe = \
        pandas.read_json(
            pylint_results_as_json)

    pylint_results_as_dataframe.rename(
        columns={'message-id': 'message_id'},
        inplace=True)
