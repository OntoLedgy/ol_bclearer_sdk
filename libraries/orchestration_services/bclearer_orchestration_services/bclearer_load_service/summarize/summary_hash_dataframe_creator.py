...
from nf_common.code.services.bclearer_load_service.common_knowledge.bclearer_load_constants import \
    ...
    IDENTITY_HASH_SET_CONTENT_HASH_NAME, NAMES_SUMMARY_COLUMN_NAME, HASHES_SUMMARY_COLUMN_NAME
...
def create_summary_hash_dataframe(
    ...
    summary_hash_dataframe = \
        pandas.DataFrame(
            columns=[NAMES_SUMMARY_COLUMN_NAME, HASHES_SUMMARY_COLUMN_NAME])

    summary_hash_dataframe = \
        __add_summary_row_if_required(
...
def __add_summary_row_if_required(
    ...
    hash_components = \
        list(hashified_and_filtered_target_dataframe[source_column_name])
